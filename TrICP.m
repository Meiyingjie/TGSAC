function [R, t, TData, iMSEi, TCorr, Dthr] = TrICP(Model, TData, MoveStep, TrMin)
...
while ((CurrStep <MoveStep)&(abs(PminPhi-minPhi)>10^(-6)))
    ...
    [R, t, TCorr, TData] = CalRtPhi(TData, SortCorrTD, TriKSI, Model); 
end


%%%%%%%%%%%%%%%%%%%%Integrated Function%%%%%%%%%%%%%%%%%%%%
%% Calculate R,t,Phi based on current overlap parameter
function [R, t,TCorr,TData] = CalRtPhi(TData,SortCorrTD, Tr, Model)
...
[R, t] = reg(TCorr, Model, TData);
% To obtain the transformation data
TData = transform_to_global(TData, R, t);
